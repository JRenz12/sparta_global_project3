class CohortsController < ApplicationController
  ...
  def show
    id = params[:id]
    @students_id = User.where(cohort_id: id)
    @trainers_id = User.where(role_id: 2 , cohort_id: id)
  end
  ...
  private
    ...
    def cohort_params
      params.require(:cohort).permit(:name, :start_date, :end_date, :course_id, :students_array, :trainers_array)
    end
end
